...
Vector3 get_line_intersection(Vector2 v1_start, Vector2 v1_end,
                              Vector2 v2_start, Vector2 v2_end) {
  float uA = ((v2_end.x - v2_start.x) * (v1_start.y - v2_start.y) -
              (v2_end.y - v2_start.y) * (v1_start.x - v2_start.x)) /
             ((v2_end.y - v2_start.y) * (v1_end.x - v1_start.x) -
              (v2_end.x - v2_start.x) * (v1_end.y - v1_start.y));
  float uB = ((v1_end.x - v1_start.x) * (v1_start.y - v2_start.y) -
              (v1_end.y - v1_start.y) * (v1_start.x - v2_start.x)) /
             ((v2_end.y - v2_start.y) * (v1_end.x - v1_start.x) -
              (v2_end.x - v2_start.x) * (v1_end.y - v1_start.y));
...
bool check_entity_collision(Entity *en_1, Entity *en_2) {
  ...
  if (en_1->is_valid && en_2->is_valid) {
    if (en_1->collider == COLL_rect && en_2->collider == COLL_rect) {
      if (en_1->pos.x < en_2->pos.x + en_2->size.x &&
      ...
      Vector2 end_1 =
          get_line_endpoint(en_1->pos, en_1->size.x, to_radians(en_1->angle));
      Vector2 end_2 =
          get_line_endpoint(en_2->pos, en_2->size.x, to_radians(en_2->angle));
      ...
      Vector2 end_1 =
          get_line_endpoint(en_1->pos, en_1->size.x, to_radians(en_1->angle));
      if (get_line_intersection(en_1->pos, end_1, en_2->pos,
                                v2(en_2->pos.x + en_2->size.x, en_2->pos.y))
              .z) {
        collision_detected = true;
      }
      if (get_line_intersection(en_1->pos, end_1, en_2->pos,
                                v2(en_2->pos.x, en_2->pos.y + en_2->size.y))
              .z) {
        collision_detected = true;
      }
      if (get_line_intersection(
              en_1->pos, end_1, v2(en_2->pos.x + en_2->size.x, en_2->pos.y),
              v2(en_2->pos.x + en_2->size.x, en_2->pos.y + en_2->size.y))
              .z) {
        collision_detected = true;
      }
      if (get_line_intersection(
              en_1->pos, end_1, v2(en_2->pos.x, en_2->pos.y + en_2->size.y),
              v2(en_2->pos.x + en_2->size.x, en_2->pos.y + en_2->size.y))
              .z) {
        collision_detected = true;
      }
      ...
      Vector2 end_2 =
          get_line_endpoint(en_2->pos, en_2->size.x, to_radians(en_2->angle));
      if (get_line_intersection(en_2->pos, end_2, en_1->pos,
                                v2(en_1->pos.x + en_1->size.x, en_1->pos.y))
              .z) {
        collision_detected = true;
      }
      if (get_line_intersection(en_2->pos, end_2, en_1->pos,
                                v2(en_1->pos.x, en_1->pos.y + en_1->size.y))
              .z) {
        collision_detected = true;
      }
      if (get_line_intersection(
              en_2->pos, end_2, v2(en_1->pos.x + en_1->size.x, en_1->pos.y),
              v2(en_1->pos.x + en_1->size.x, en_1->pos.y + en_1->size.y))
              .z) {
        collision_detected = true;
      }
      if (get_line_intersection(
              en_2->pos, end_2, v2(en_1->pos.x, en_1->pos.y + en_1->size.y),
              v2(en_1->pos.x + en_1->size.x, en_1->pos.y + en_1->size.y))
              .z) {
        collision_detected = true;
      }
    } else if (en_1->collider == COLL_point && en_2->collider == COLL_rect) {
      if (en_1->pos.x >= en_2->pos.x &&
...
bool check_entity_will_collide(Entity *en_1, Entity *en_2) {
  ...
  en_1->pos =
      v2_add(en_1->pos, v2_mulf(en_1->move_vec, en_1->move_speed * delta_t));
  en_2->pos =
      v2_add(en_2->pos, v2_mulf(en_2->move_vec, en_2->move_speed * delta_t));
  collision_detected = check_entity_collision(en_1, en_2);

  en_1->pos =
      v2_sub(en_1->pos, v2_mulf(en_1->move_vec, en_1->move_speed * delta_t));
  en_2->pos =
      v2_sub(en_2->pos, v2_mulf(en_2->move_vec, en_2->move_speed * delta_t));
...
void solid_entity_collision(Entity *en_1, Entity *en_2) {
  ...
  if (check_entity_collision(en_1, en_2)) {
    Vector2 en_to_en_vec =
        v2_sub(get_entity_midpoint(en_1), get_entity_midpoint(en_2));
    if (!en_1->is_static) {
      en_1->pos =
          v2_add(en_1->pos, v2_mulf(v2_normalize(en_to_en_vec), delta_t));
    }
  }
...
bool check_ray_collision(Vector2 ray, Entity *en_1, Entity *en_2) {
  ...
  if (en_1->pos.x < en_2->pos.x + en_2->size.x &&
    ...
    collision_detected = true;
  } else if (ray.x < en_2->pos.x + en_2->size.x &&
             en_1->pos.x + en_1->size.x + ray.x > en_2->pos.x &&
             en_1->pos.y < en_2->pos.y + en_2->size.y &&
             en_1->pos.y + en_1->size.y + ray.y > en_2->pos.y) {
    collision_detected = true;
             ...
             en_1->pos.y + en_1->size.y + ray.y > en_2->pos.y) {
    collision_detected = true;
  } else if (en_1->pos.x < en_2->pos.x + en_2->size.x &&
             en_1->pos.x + en_1->size.x + ray.x > en_2->pos.x &&
             ray.y < en_2->pos.y + en_2->size.y &&
             en_1->pos.y + en_1->size.y + ray.y > en_2->pos.y) {
    collision_detected = true;
...
bool world_save_to_disk() {
  return os_write_entire_file_s(STR("world"),
                                (string){sizeof(World), (u8 *)world});
}
bool world_attempt_load_from_disk() {
  ...
  // gracefully fail somehow. That's why this function returns a bool. We handle
  // that at the callsite. Maybe we want to just start up a new world, throw a
  // user friendly error, or whatever as a fallback. Not just crash the game
  // lol.
...
void setup_world() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    monster_en->pos = v2_rotate_point_around_pivot(
        monster_en->pos, v2(0, 0), get_random_float32_in_range(0, 2 * PI64));
...
void render_line_entity(Entity *en) {
  if (en->is_valid) {
    Vector2 endpoint =
        get_line_endpoint(en->pos, en->size.x, to_radians(en->angle));
...
void set_screen_space() {
  ...
  draw_frame.projection = m4_make_orthographic_projection(
      0.0, screen_width, 0.0, screen_height, -1, 10);
}
...
Vector2 world_to_screen(Vector2 p) {
  Vector4 in_cam_space =
      m4_transform(draw_frame.camera_xform, v4(p.x, p.y, 0.0, 1.0));
  ...
  return v2((ndc.x + 1.0f) * 0.5f * (f32)window.width,
...
Vector2 world_size_to_screen_size(Vector2 s) {
  ...
  return v2(screen_size_point.x - screen_origin.x,
...
Draw_Quad ndc_quad_to_screen_quad(Draw_Quad ndc_quad) {
  ...
  ndc_quad.bottom_left = m4_transform(ndc_to_screen_space,
                                      v4(v2_expand(ndc_quad.bottom_left), 0, 1))
                             .xy;
  ndc_quad.bottom_right =
      m4_transform(ndc_to_screen_space,
                   v4(v2_expand(ndc_quad.bottom_right), 0, 1))
          .xy;
  ndc_quad.top_left =
      m4_transform(ndc_to_screen_space, v4(v2_expand(ndc_quad.top_left), 0, 1))
          .xy;
  ndc_quad.top_right =
      m4_transform(ndc_to_screen_space, v4(v2_expand(ndc_quad.top_right), 0, 1))
          .xy;
  ...
  for (int i = 0; i < ARRAY_COUNT(particles); i++) {
    ...
    if (p->flags & PARTICLE_FLAGS_physics) {
      if (p->flags & PARTICLE_FLAGS_friction) {
        p->acceleration =
            v2_sub(p->acceleration, v2_mulf(p->velocity, p->friction));
      }
...
void particle_render() {
  for (int i = 0; i < ARRAY_COUNT(particles); i++) {
    ...
    if (p->flags & PARTICLE_FLAGS_fade_out_with_velocity) {
      col.a *=
          float_alpha(fabsf(v2_length(p->velocity)), 0, p->fade_out_vel_range);
    }
...
void particle_emit(Vector2 pos, ParticleKind kind) {
  ...
  case PFX_hit: {
    for (int i = 0; i < 4; i++) {
      ...
      p->flags |= PARTICLE_FLAGS_physics | PARTICLE_FLAGS_friction |
                  PARTICLE_FLAGS_fade_out_with_velocity;
      p->pos = pos;
      p->velocity = v2_normalize(v2(get_random_float32_in_range(-1, 1),
                                    get_random_float32_in_range(-1, 1)));
...
int entry(int argc, char **argv) {
...

  {
    sprites[0] = (Sprite){
        .image = load_image_from_disk(
            fixed_string("res\\sprites\\undefined.png"), get_heap_allocator())};
    sprites[SPRITE_player] = (Sprite){
        .image = load_image_from_disk(fixed_string("res\\sprites\\player.png"),
                                      get_heap_allocator())};
    sprites[SPRITE_monster] = (Sprite){
        .image = load_image_from_disk(fixed_string("res\\sprites\\monster.png"),
                                      get_heap_allocator())};
    sprites[SPRITE_experience] = (Sprite){
        .image = load_image_from_disk(fixed_string("res\\sprites\\sword.png"),
                                      get_heap_allocator())};
  ...
  debug_render = true;
  font = load_font_from_disk(STR("C:/windows/fonts/arial.ttf"),
                             get_heap_allocator());
  ...
  while (!window.should_close) {
    ...
    for (int i = 0; i < MAX_ENTITY_COUNT; i++) {
      Entity *en = &world->entities[i];
      if (!en->is_valid) {
        // clean up flagged entities before these pointers get used and after
        // render
        entity_destroy(en);
    ...
    world_frame.world_proj = m4_make_orthographic_projection(
        window.width * -0.5, window.width * 0.5, window.height * -0.5,
    ...
    // :camera
    {
      ...
      // randy: these might be ordered incorrectly for the camera shake. Not
      // sure.

      // translate into position
      world_frame.world_view = m4_translate(world_frame.world_view,
                                            v3(camera_pos.x, camera_pos.y, 0));

      // translational shake
      float shake_x = max_cam_shake_translate * cam_shake *
                      get_random_float32_in_range(-1, 1);
      float shake_y = max_cam_shake_translate * cam_shake *
                      get_random_float32_in_range(-1, 1);
      world_frame.world_view =
          m4_translate(world_frame.world_view, v3(shake_x, shake_y, 0));
      ...
      // scale the zoom
      world_frame.world_view =
          m4_scale(world_frame.world_view, v3(1.0 / zoom, 1.0 / zoom, 1.0));
    ...
    //: input
    {
      ...
      if (v2_length(get_player()->input_axis) != 0) {
        get_player()->angle = v2_angle(get_player()->input_axis, v2(1, 0));
      }
    }
      ...
      for (int i = 0; i < MAX_ENTITY_COUNT; i++) {
        ...
        if (en->is_valid) {
          ...
          case ARCH_player:
            ...
            en->pos = v2_add(en->pos,
                             v2_mulf(en->move_vec, en->move_speed * delta_t));
            ...
            //: hp
            {
              ...
              Matrix4 xform = m4_scalar(1.0);
              xform = m4_translate(
                  xform, v3(get_player()->pos.x, get_player()->pos.y, 0));
              draw_rect_xform(xform, v2(10, -5), COLOR_RED);
              draw_rect_xform(
                  xform,
                  v2((get_player()->health.current / get_player()->health.max) *
                         10.0f,
                     -5),
                  COLOR_GREEN);
              pop_z_layer();
          ...
          case ARCH_weapon:
            ...
            if (get_player()->experience.current >=
                get_player()->experience.max) {
          ...
          case ARCH_monster:
            ...
            if (debug_render) {
              draw_line(en->pos,
                        v2_add(en->pos, v2_mulf(en->move_vec, tile_width)), 1,
                        COLOR_RED);
            }
            ...
            string text =
                sprint(temp_allocator, STR("%f %f"), en->pos.x, en->pos.y);
            push_z_layer(layer_text);
            Matrix4 xform = m4_scalar(1.0);
            xform = m4_translate(xform, v3(en->pos.x, en->pos.y, 0));
            // draw_text_xform(font, text, font_height, xform, v2(0.1, 0.1),
            // COLOR_YELLOW);
            pop_z_layer();
            ...
            if (en->pos.x - camera_pos.x < -screen_width * 2 ||
              ...
              en->color = v4(0, 0, 0, 0);
              en->is_valid = false;
              // log("destroyed offscreen monster at screen pos %f %f",
              // en->pos.x, en->pos.y);
            }
            break;
          case ARCH_pickup:
            set_world_space();
            push_z_layer(layer_entity);
            en->move_vec = v2_sub(get_entity_midpoint(get_player()),
                                  get_entity_midpoint(en));
            en->move_vec = v2_normalize(en->move_vec);

            if (fabsf(v2_dist(get_entity_midpoint(en),
                              get_entity_midpoint(get_player()))) <
                tile_width * 2.0) {
              en->move_speed = 165;
            }
        ...
        for (int y = player_tile_y - tile_radius_y;
          ...
          if ((x + (y % 2 == 0)) % 2 == 0) {
            ...
            draw_rect(v2(x_pos + tile_width * -0.5, y_pos + tile_width * -0.5),
                      v2(tile_width, tile_width), col);
          }
        }
      }

      pop_z_layer();
      // draw_rect(v2(tile_pos_to_world_pos(mouse_tile_x) + tile_width * -0.5,
      // tile_pos_to_world_pos(mouse_tile_y) + tile_width * -0.5),
    ...
    if (world->ux_state == UX_win) {
      ...
      Matrix4 xform = m4_scalar(1.0);
      xform =
          m4_translate(xform, v3(screen_width / 4.0, screen_height / 2.0, 0));
      draw_text_xform(font, text, font_height, xform, v2(0.5, 0.5),
                      COLOR_YELLOW);
    ...
    //: bar rendering
    {
      ...
      draw_rect_xform(
          xform,
          v2((get_player()->experience.current / get_player()->experience.max) *
                 screen_width,
             10),
          COLOR_RED);
      ...
      draw_rect_xform(
          xform,
          v2((get_player()->health.current / get_player()->health.max) * 25.0f,
             0.5),
          COLOR_GREEN);
      draw_text_xform(font,
                      sprint(temp_allocator, STR("%.0f/%.0f"),
                             get_player()->health.current,
                             get_player()->health.max),
    ...
    if (debug_render) {
      {
        ...
        if (seconds_counter > 1.0) {
          ...
          if (world->ux_state != UX_lose) {
            for (int i = 0; i < 40; i++) {
              ...
              setup_monster(monster_en);
              monster_en->pos =
                  v2(get_random_int_in_range(5, 15) * tile_width, 0);
              monster_en->pos = v2_rotate_point_around_pivot(
                  monster_en->pos, v2(0, 0),
        ...
        Matrix4 xform = m4_scalar(1.0);
        xform =
            m4_translate(xform, v3(0, screen_height - (font_height * 0.1), 0));
        draw_text_xform(font, text, font_height, xform, v2(0.1, 0.1),
                        COLOR_RED);
      }
    }
    ...
    // load/save commands
    // these are at the bottom, because we'll want to have a clean spot to do
    // this to avoid any mid-way operation bugs.
